...
export class FitbitPollingService {
  ...
  /**
   * Backfill missing data from last DB timestamp to today
   * Called on startup to catch up after downtime
   */
  async backfill(): Promise<void> {
    if (!this.initialized || !this.client.isAuthenticated()) return;

    console.log("[Fitbit] 🔄 Checking for missing data to backfill...");

    const today = new Date();
    const maxBackfillDays = 150; // Fitbit intraday data limit
    ...
    // Calculate days to backfill for each data type
    ...
    console.log(`[Fitbit] 🔄 Backfilling ${daysToBackfill} days of data...`);
    console.log(`[Fitbit]   HR: ${hrDays}d, Steps: ${stepsDays}d, HRV: ${hrvDays}d, Sleep: ${sleepDays}d, Temp: ${tempDays}d`);

    // Backfill each missing day (oldest first)
    for (let daysAgo = daysToBackfill - 1; daysAgo >= 0; daysAgo--) {
      ...
      try {
        // Heart Rate
        const heartRateData = await this.client.getHeartRate(date);
        if (heartRateData && heartRateData.readings.length > 0) {
          ...
          if (heartRateData.restingHeartRate !== null) {
            await insertFitbitRestingHeartRate(config.userId, date, heartRateData.restingHeartRate);
          }
        }

        // Steps
        const stepsIntraday = await this.client.getStepsIntraday(date);
        if (stepsIntraday.length > 0) {
        ...
        const [hrvDaily, sleepSessions, temperature] = await Promise.all([
          this.client.getHrvDaily(date),
          this.client.getSleep(date),
          this.client.getTemperature(date),
        ]);
        ...
        // Small delay to avoid rate limits (150 requests/hour)
        if (daysAgo > 0) {
   ...
   * Runs backfill first to catch up on any missing data
   */
  async startPolling(): Promise<void> {
    ...
    // Backfill any missing data first
    await this.backfill();
