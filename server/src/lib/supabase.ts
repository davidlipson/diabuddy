...
/**
 * Get the latest HRV daily date for a user (for backfill)
 */
export async function getLatestFitbitHrvDate(
  userId: string,
): Promise<Date | null> {
  const supabase = getSupabase();
  ...
  if (error) {
    if (error.code === "PGRST116") return null; // No rows
    throw new Error(`Failed to get latest HRV date: ${error.message}`);
  }
...
/**
 * Get the latest sleep session date for a user (for backfill)
 */
export async function getLatestFitbitSleepDate(
  userId: string,
): Promise<Date | null> {
  const supabase = getSupabase();
  ...
  if (error) {
    if (error.code === "PGRST116") return null; // No rows
    throw new Error(`Failed to get latest sleep date: ${error.message}`);
  }

  return data ? new Date(data.date_of_sleep) : null;
}

/**
 * Get the latest temperature date for a user (for backfill)
 */
export async function getLatestFitbitTemperatureDate(
  userId: string,
): Promise<Date | null> {
  const supabase = getSupabase();
  ...
  if (error) {
    if (error.code === "PGRST116") return null; // No rows
    throw new Error(`Failed to get latest temperature date: ${error.message}`);
  }
